...
use ratatui::{backend::Backend, style::{Color, Modifier, Style}, text::{Line, Span}, widgets::{block::Title, Block, Borders, List}, Terminal};
use sqlx::MySqlPool;
            ...
            if let Event::Key(key_event) = read()? {
                if key_event.kind == KeyEventKind::Press {
                    match key_event.code {
                        KeyCode::Char('c') => {
                            // Create a new member
                            match CreateRecordDialog::new(vec!["First Name".into(), "Last Name".into(), "Email".into(), "Phone".into()], |d: &CreateRecordDialog| true).run(terminal, pool).await? {
                        ...
                        KeyCode::Char('e') => {
                            ...
                            let current_data = vec![current_member.first_name.clone(), current_member.last_name.clone(), current_member.email.clone(), current_member.phone.clone()];
        
                            match CreateRecordDialog::new_edit(vec!["First Name".into(), "Last Name".into(), "Email".into(), "Phone".into()], current_data, |d: &CreateRecordDialog| true).run(terminal, pool).await? {
                                CreateResults::Create(data) => {
                        ...
                        KeyCode::Char('d') => {
                            if ConfirmDelete::run(terminal).await {
                                ...
                                let delete_contributions_query = "DELETE FROM ContributesTo WHERE MemberID = ?";
    ...
    fn draw(&self, terminal: &mut Terminal<impl Backend>) -> std::io::Result<()> {
        terminal.draw(|f| {
            ...
            let title_block = Block::default()
                .title(instructions_title.alignment(ratatui::layout::Alignment::Center).position(ratatui::widgets::block::Position::Bottom))
                .borders(Borders::ALL)
                .title_alignment(ratatui::layout::Alignment::Center);
    
            // Create a list of member entries
            let members: Vec<Line> = self.members.iter().enumerate().map(|(index, member)| {
                ...
                let line = Line::from(vec![
                    ...
                    Span::styled(format!("{} {} ", member.first_name, member.last_name), style),
